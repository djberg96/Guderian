...
RSpec.describe Map::SmolenskData do
  describe ".apply!" do
    it "applies the entered terrain, river, road, and railroad data for rows 01 and 02" do
      ...
      third_river = HexsideFeature.find_by!(hex: Hex.find_by!(hex_number: "0212"), direction: "northeast", feature_type: "river")
      expect(third_river).to be_present

      fourth_river = HexsideFeature.find_by!(hex: Hex.find_by!(hex_number: "0219"), direction: "northeast", feature_type: "river")
      ...
      expect(Hex.find_by!(hex_number: "0209").railroad_exits).to eq(%w[northeast])
      expect(Hex.find_by!(hex_number: "0210").railroad_exits).to eq(%w[northeast southeast])
      expect(Hex.find_by!(hex_number: "0228").railroad_exits).to eq(%w[northeast])
      expect(Hex.find_by!(hex_number: "0229").railroad_exits).to eq(%w[southwest northeast])
      expect(Hex.find_by!(hex_number: "0230").railroad_exits).to eq(%w[southwest])
    end
  end
end
